...
- **AI 算法与信号处理层**：基于 Python 3.11。`Librosa` 执行音频数字信号处理（DSP）：重采样、Mel 频谱图提取与 RMS 能量窗口筛选；`PyTorch` 部署自研的深度哈希网络（DHN）——`AudioHashNet`，将高维声学特征压缩为 128 位二进制指纹。
- **高速向量检索层**：采用 Facebook 开源的 `Faiss` 引擎，基于 `IndexBinaryFlat` 算法实现针对 20 万+ 二进制哈希码的毫秒级汉明距离（Hamming Distance）极速匹配。
- **全栈 Web 后端**：`FastAPI` + `Uvicorn` 作为高性能异步服务器，处理路由分发与 CORS；`SQLite` 主库 `music_hash.db` 存储歌曲元数据、用户档案、收藏夹与识别历史；引入 `bcrypt` 做密码哈希 + `PyJWT` 实现无状态 7 天有效期访问令牌，前端主要通过 httpOnly Cookie 携带登录态，后端兼容 Bearer Token；所有敏感端点（收藏、历史）均通过 `get_current_user(...)` 从 JWT payload 提取 `user_id`，**不信任前端传值**，从根本上阻断水平越权。
...
3. **领域鸿沟（Domain Gap）与抗噪鲁棒性**：详细论证了干净训练数据集与真实外录场景（设备底噪、频段丢失、房间混响）的分布差异。当前实现重点保证在线查询与离线建库预处理对称，并通过白噪声在线增强提升加性噪声条件下的泛化能力；端点检测、音量归一化、SpecAugment 等更激进方案保留为后续研究路线，避免破坏当前已验证的特征分布。
...
1. **JWT 无状态鉴权体系**：废弃早期基于 cookie-session 的简易方案，引入 `PyJWT` HS256 签发 7 天有效期 JWT；前端主要通过 httpOnly Cookie 自动携带登录态，后端也兼容 Bearer Token。所有涉及用户数据的端点（`/favorite/toggle`、`/favorites/{uid}`、`/history`）强制走 `get_current_user()` 提取身份，避免水平越权。生产 HTTPS 部署显式设置 `COOKIE_SECURE=1`。
2. **速率限制与测试覆盖**：`tests/` 目录搭载 6 个测试文件（`conftest.py` + `test_auth.py` + `test_jwt.py` + `test_preprocessing.py` + `test_search.py` + `test_batch_extract.py`），覆盖注册登录边界条件、JWT 签发/过期/伪造、音频预处理管线、在线检索聚合、离线建库失败回滚、防刷速率限制等关键契约。
3. **识别历史时间胶囊**：`search_history` 表记录用户每次查询的源音频、命中结果、汉明距离、发生时间，前端通过 `GET /history` 展示可回溯列表、`DELETE /history` 支持一键清空。
4. **全局迷你播放器**：前端实现一个底部播放控件，用单个 HTML Audio 实例承接搜索结果和收藏列表里的试听操作；`localStorage` 只保存展示用用户信息，不保存 JWT 或播放进度。
